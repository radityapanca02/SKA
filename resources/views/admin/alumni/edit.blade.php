<x-admin-layout>
    <h1 class="text-xl sm:text-2xl font-bold mb-4">Edit Alumni</h1>
    ...
    <form action="{{ route('admin.alumni.update', $alumnus->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $alumnus->name) }}" class="w-full border rounded p-2 text-sm" required maxlength="100">
            </div>

            <div>
                <label class="block mb-1 text-sm font-medium">Tahun Kelulusan</label>
                <input type="text" name="graduation" value="{{ old('graduation', $alumnus->graduation) }}" class="w-full border rounded p-2 text-sm" required maxlength="50" placeholder="Alumni TP 2016">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <div>
                ...
                <input type="text" name="position" value="{{ old('position', $alumnus->position) }}" class="w-full border rounded p-2 text-sm" required maxlength="100">
            </div>

            <div>
                <label class="block mb-1 text-sm font-medium">Perusahaan</label>
                <input type="text" name="company" value="{{ old('company', $alumnus->company) }}" class="w-full border rounded p-2 text-sm" required maxlength="100">
            </div>
        </div>

        <div>
            <label class="block mb-1 text-sm font-medium">Deskripsi</label>
            <textarea name="description" rows="3" class="w-full border rounded p-2 text-sm" required>{{ old('description', $alumnus->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            ...
            <div>
                <label class="block mb-1 text-sm font-medium">Foto Saat Ini</label>
                <div class="mb-2">
        ...
        <div>
            ...
            <textarea name="achievements" rows="3" class="w-full border rounded p-2 text-sm" placeholder="Employee of the Year 2020&#10;Peningkatan Produktivitas 35%">
@php
...
@endphp
        ...
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
            ...
            <a href="{{ route('admin.alumni.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 text-sm text-center">
